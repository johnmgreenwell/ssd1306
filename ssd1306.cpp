 ...
 * 'bitbang' software SPI are both supported (in this modified version,
 * by way of HAL interface; see below).
 *
...
// TODO: this is way too much to allocate to a temp buf, restructure below
#define WIRE_MAX        (32u) ///< Use common Arduino core default
#define I2C_BUFFER_SIZE (WIRE_MAX)
...
/*!
    ...
    @param  i2c
            Pointer to an existing HAL::I2C bus instance on which SSD1306
...
{
    i2c_buffer[0] = (uint8_t)0x00; // Co = 0, D/C = 0
    i2c_buffer[1] = c;

    _i2c.write(i2caddr, i2c_buffer, 2);
}
...
void SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n)
{
    ...
    while (n--)
    {
        ...
        {
            _i2c.write(i2caddr, i2c_buffer, bytesOut);
            bytesOut = 1;
        }

        i2c_buffer[bytesOut++] = pgm_read_byte(c++);
    }
...
#if not(defined(SSD1306_NO_SPLASH))
    if (HEIGHT > 32) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void SSD1306::drawFastHLine(int16_t x, int16_t y, int16_t w,
                                     uint16_t color)
{
    bool bSwap = false;
    switch (rotation)
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void SSD1306::drawFastVLine(int16_t x, int16_t y, int16_t h,
                                     uint16_t color)
{
    bool bSwap = false;
    switch (rotation)
...
/*!
   ...
   SSD1306_INVERSE.
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void SSD1306::drawFastVLineInternal(int16_t x, int16_t __y,
